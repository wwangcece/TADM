...
# https://github.com/openai/guided-diffusion/blob/main/guided_diffusion/image_datasets.py
def random_crop_arr(pil_images, image_size):
    pil_images = [np.array(pil_image) for pil_image in pil_images]
    crop_y = random.randrange(pil_images[0].shape[0] - image_size + 1)
    crop_x = random.randrange(pil_images[0].shape[1] - image_size + 1)
    return (
        [
            arr[crop_y : crop_y + image_size, crop_x : crop_x + image_size]
            for arr in pil_images
        ],
        crop_y,
        crop_x,
    )
